...
完成 `weeks/week-01/pre-class.md` 的闭卷桥接。若不能写出 `QK^T` 与
`weights @ V` 的 shape，回到 Week 0；不要直接进入 Lab。

## Gate 1：读懂信息流（P1，T1）

完成材料中的四个问题和 exercises 的 Gate 1。产物是一张 shape 表和一次
“只改 V”预测。
...
完成 exercises 的 Gate 2，并构造一次转置错误的反例。

**解锁条件：** 能解释 `scores[i,j]`、每行竞争关系和 `K.T` 的必要性。

## Gate 3：建立权重契约（P0，T1 → T4）

完成 exercises 的 Gate 3。必须留下一个极端数值例子和一个错误 axis 的
二维例子。
...
完成 `weeks/week-01/homework.md` 与 `notes-template.md`，然后做一个独立的
