...
\hypersetup{pdftitle={Week 0 Problem Set - 先看懂信息怎样流动}}

\begin{document}
\CourseCover
  {Week 0 Problem Set}
  {先看懂信息怎样流动}
  {用推导、反例和真实 Lab 证据解释矩阵 shape、点积与 stable softmax。}
  {Week 1：Q/K/V、score 与内容读取。}

\section{课程位置与使用规则}

本题集不要求实现 attention。目标是在 Week 1 前，准确追踪 token 表、矩阵乘法、点积和 stable softmax 的输入、输出与 shape。

\textbf{提交物：}书面解答、Shape Checker Lab、一个真实错误或主动注入错误的诊断记录，以及闭卷信息流图。

\textbf{使用规则：}Part A 与 Part B 必须在进入 Lab 前独立完成。Part C 必须在 Lab 后引用自己的输入、输出、测试或评分反馈。本题集不包含答案或实现代码。

\section{Part A：概念与推导}
...
\section{Part C：Lab 后工程问题}

\Question{C1. Shape Checker 的契约（工程设计）}
为什么要将 shape 验证与输出 shape 推导拆成两个函数？分别为 \Code{require\_matrix} 和 \Code{matrix\_shape} 写出一个它单独负责的错误场景。
\AnswerSpace{25mm}

\Question{C2. 一个真实错误（工程诊断）}
选择一次 Lab 失败或主动注入错误 shape。记录输入、现象、错误类别、原因假设、最小验证测试，以及修复后理解的变化。
\AnswerSpace{32mm}
