...
Markdown、YAML、Lab 和 grader 是可维护、可验证的课程源；正式面向学习者发布
的阅读与书面练习产物使用 PDF。Runtime 读取源文件和结构化证据，不解析 PDF，
避免把排版结果变成机器状态真相。

每个可发布阶段至少提供两份 PDF：

```text
resources/week-XX.pdf                    Learning Guide：课程位置、前置能力、
                                         精确材料、阅读问题、Gate 导航与证据清单
problem-sets/week-XX-problem-set.pdf     Problem Set：概念推导、边界反例、
                                         Lab 后工程题、迁移题与提交清单
```

完成后讲评使用独立的 `solutions/week-XX-solutions.pdf`，不得混入 Learning Guide
或 Problem Set。公开仓库中的 README 负责定位与下载，PDF 负责正式学习体验；
Markdown 源仍保留，便于版本控制、Agent 读取和无障碍替代访问。

所有 PDF 复用统一视觉模板，并满足：

- A4 页面、稳定页边距、页码、清楚的中英文字体和一致标题层级；
- 封面说明本周能力、前置、完成后解锁内容和预计提交物；
- 目录或清楚的 Part 导航、可点击且可读的外部链接；
- 公式使用数学环境，代码与命令使用等宽字体；
- Problem Set 为手写或电子批注保留空间，答案与题目分离；
- 每个 Gate 明确“做什么、交什么证据、失败回到哪里”；
- 不出现文字裁切、重叠、黑块、孤立标题、不可读断行或表格越界；
- 编译后必须逐页渲染为 PNG 做视觉检查，并抽取文本检查页数、标题和链接；
- PDF 构建具有可重复命令，生成失败或视觉检查未通过时不得发布。

Week 0 现有 PDF 作为遗留出版物处理：内容可以复用，但在统一模板、自动构建和
逐页视觉检查通过前不视为正式发布基线。Week 1 是新发布契约的第一份完整实现。
...
- Week 1 的 Learning Guide 与 Problem Set PDF 使用统一模板生成，README 提供
  清楚入口，且源文件与 PDF 标题、Gate、提交物和解锁条件一致；
- 所有正式 PDF 已逐页渲染检查，未发现裁切、重叠、断字、黑块、表格越界或
  不可读链接；
