...
本功能只实现未来 Agent 必须复用的工具边界，不继续模拟完整 Agent 对话：

- 一个通用 Markdown 模板按 Gate 元数据生成不同作答文件，不维护七份重复模板；
- Answer Workspace 只负责初始化和结构检查，不评价答案正确性；
- Git Guard 只证明当前作答已经 commit，不管理 Git 工作流；
- CLI 只是这些能力的临时调用壳；未来 Agent 直接把 Workspace、Git Guard、Ledger、
  Policy 和 State Machine 作为 tools 使用；
- 分支命名策略、模板版本迁移、OCR、开放回答判分和个性化文案延后。

因此需要长期迁移的是证据 schema、只读 Git provenance、Ledger 与状态授权边界，
不是当前 CLI 的步骤或输出文字。
...
- 一个通用模板初始化 Week 1 Gate 0–6 的独立作答文件；
- 学生分支检查和命名警告；
- 当前 Gate 文件按需初始化；
...
| 作答模板 | `resources/week-01/answer-template.md` | 尚无 | new required role | 一个通用模板按 Gate 初始化，归入现有 resource 角色 |
...
```text
resources/week-XX/answer-template.md
...
Manifest Loader 必须验证通用模板存在、artifact path 不逃逸 `answer_root`、Gate 路径唯一、
...
- 一个通用模板能够按 Manifest 初始化 Week 1 七个 Gate 的独立作答路径；
